...
6. 如果 `.coder/hooks/post-close`（或 `.js`）存在，把更新後的完整任務 row（含 `status: "DONE"`、`closedAt`）包成 `{"task": {...}}` 寫進一個暫存檔、路徑當參數傳給它並執行（細節見下方 Hooks 章節）
...
範例 hook（`task-fetch`、`format-commit-msg`、`post-close`）都用 `jq` 解析/組裝 JSON，執行環境要有裝 `jq`。

### 全域規則（三支 hook 共用）

- **資料一律靠檔案就地讀寫，不走 stdin/stdout pipe**——這是抄 Git 自己的 `commit-msg`/`prepare-commit-msg` hook 慣例：呼叫者丟一個檔案路徑給 hook，hook 直接讀/改那個檔案，呼叫者事後把檔案讀回來，而不是靠 hook 把資料印到 stdout 讓呼叫者解析。需要輸入資料的 hook（`format-commit-msg`、`post-close`）會拿到一個已經寫好內容的暫存檔路徑；需要輸出資料的 hook（`task-fetch`、`format-commit-msg`）要把結果寫回（覆蓋）同一個檔案。檔案路徑一律當第一個 CLI 參數（`$1`）傳入。
- **檔案內容一律是 JSON**，不管輸入還是輸出、不管資料形狀是陣列、物件還是單一字串——即使 `format-commit-msg` 的回傳只是一個 commit message 字串，也包成 `{"message": "..."}`，而不是直接寫純文字。這樣三支 hook 的資料交換方式完全一致，不用每支另外記格式。細節看下面各 hook 的 Request/Response。
- **stdin/stdout/stderr 三個通道全部原封不動繼承你的終端機，coder 完全不讀也不寫。** 三支 hook 都可以自由印 log/進度條/debug 訊息到任何地方，也都可以用 `read` 之類的方式跟執行 `coder` 的人互動（確認提示、填表單、輸入 token…）——不會有任何一支因為「資料通道被佔用」而不能互動。
- 任何一支 hook 該寫回的檔案內容不符合下方該 hook 講的格式，`coder` 都會直接報錯中止，不會把不明內容當成資料使用。
...
- **觸發時機**：`coder fetch`
- **Request（CLI 參數）**：coder 建立一個空的暫存檔，把路徑當第一個參數傳進去（`$1`）。沒有其他輸入。
- **Response（資料檔）**：hook 執行完後，coder 讀回 `$1` 那個檔案，整段內容必須「就是」：
  ```jsonc
  {
    "tasks": [
      {
  ...
  ```
- **錯誤情境**：exit code 非 0、資料檔為空、內容不是合法 JSON、`tasks` 不是陣列、陣列元素缺必填欄位或 `status` 不合法——都會讓 `coder fetch` 整批失敗（DB 不會有任何寫入，failure 是 all-or-nothing）。
...
- **Request（資料檔）**：coder 建立一個暫存檔、寫入以下 JSON，把路徑當第一個參數傳進去（`$1`）：
  ```jsonc
  {
  ...
  ```
- **Response（資料檔）**：hook 把最終結果**覆寫回同一個檔案**（`$1`），整段內容必須「就是」：
  ```jsonc
  { "message": "string" }  // 最終要拿去 commit 的訊息，可以多行
  ```
- **錯誤情境**：exit code 非 0、資料檔為空、內容不是合法 JSON、`message` 不存在或不是非空字串——都會讓 `coder commit` 中止（此時還沒有 commit，不會留下半成品）。

### `post-close` / `post-close.js`

用途：任務關閉後通知外部系統（任務追蹤工具、下游自動化…），或是跑一段需要使用者互動的流程（例如關閉任務後順手問要不要填一份 bug 報告）。選用 hook，且是 best-effort，失敗只印警告，不影響 `coder close` 本身的結果。

- **觸發時機**：`coder close`，任務合併進 `baseBranch` 並轉成 `DONE` 之後
- **Request（資料檔）**：coder 建立一個暫存檔、寫入以下 JSON，把路徑當第一個參數傳進去（`$1`）：
  ```jsonc
  {
    "task": { "id": 1, "ticketId": "string | null", "title": "string", "body": "string | null", "status": "DONE", "baseBranch": "string", "createdAt": "string", "closedAt": "string" }
  }
  ```
  stdin 完全沒被 coder 動過（不是 pipe、沒有寫入任何東西），直接繼承終端機，所以這支 hook 可以自由用 `read` 之類的方式跟使用者互動（`coder close` 會同步等待這支 hook 執行完才繼續，不是背景執行）。
- **Response**：無——這支 hook 的資料檔不會被 coder 讀回，愛寫什麼都行（stdout 也一樣不會被讀取或解析，但仍建議照全域規則走 stderr）。
