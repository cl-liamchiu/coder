...
| Hook | 何時觸發 | 必要性 | 介面 |
|---|---|---|---|
| `task-fetch` / `task-fetch.js` | `coder fetch` | 必要（沒有就會失敗） | 無輸入；整段 stdout 必須「就是」JSON 陣列 `[{ticketId?, title, body?, baseBranch, status?}]`（可以是 pretty-print 多行，但不能混入其他文字；log/進度訊息請印到 stderr）（`status` 省略時預設 `TODO`；不接受 `DONE`） |
